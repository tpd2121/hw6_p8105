...
My `fct_reorder` and `arrange` function didn’t work, so I couldn’t
organize the plot based on the OR. But we can see that in most
city-states, male victim’s cases have lower odds of being solved
compared to female victim’s. Only 3 city-states where male victim’s
cases have higher odds of being solved, which are Stockton (CA), Fresno
(CA), and Albuquerque (NM)- all of which has wide confidence interval.
