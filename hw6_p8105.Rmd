...
Should I only choose 1 variable? so smoking only OR weight gain only?
...
**comparison**
```{r cross_validation}
#the website say "It’s worth noting, though, that if all the models you want to fit use lm, you can skip this." and since my model both use lm, do I skip them?

#the masterdata become the training data, the models we build become the test data
#do i still do the 80/20 separation? or does the test data 100% from bwt_masterdata?
...
cv_df1 = cv_df1 %>% 
  mutate(
    fit_mod  = map(train, \(df) lm(bwt ~ smoken + wtgain, data = df)),
    fit2_mod  = map(train, \(df) lm(bwt ~ blength + gaweeks, data = df)),
    fit3_mod = map(train, \(df) lm(bwt ~ bhead*blength*babysex + bhead + blength + babysex, data = df))
  ) %>% 
  mutate(
